...
import 'package:luxair/otherpages/warehouseaccdetails.dart';
...
class WarehouseAcceptanceAwbList extends StatefulWidget {
  ...
  @override
  State<WarehouseAcceptanceAwbList> createState() =>
      _WarehouseAcceptanceAwbListState();
}

class _WarehouseAcceptanceAwbListState
    extends State<WarehouseAcceptanceAwbList> {
  ...
  List<WarehouseAcceptanceAWB> dockInOutVTListToBind = [];
  //List<DockInOutVT> dockInOutVTListImport = [];
  List<WarehouseAcceptanceAWB> dockInOutVTListExport = [];
  List<WarehouseAcceptanceAWB> dockInOutVTListRandom = [];
  ...
  void initState() {
    ...
    //   getDockInTokenList(1); //Import
    // else
    getWarehouseAcceptanceAWBList(2); //Export
    super.initState();
  }
  ...
  getWarehouseAcceptanceAWBList(modeType) async {
    if (isLoading) return;
                        ...
                        child: SizedBox(
                            ...
                            child: ListView.builder(
                              ...
                              itemBuilder: (BuildContext, index) {
                                WarehouseAcceptanceAWB _dockinlist =
                                    dockInOutVTListToBind.elementAt(index);
                                return buildWarehouseAWBList(
                                    _dockinlist, index);
                              },
  ...
  buildWarehouseAWBList(WarehouseAcceptanceAWB _dl, index) {
    return index < 120
        ? GestureDetector(
            onTap: () async {
              var returnVal = await Navigator.push(
                  context,
                  MaterialPageRoute(
                      builder: (context) => WarehouseAcceptanceDetails(
                            vtNumber: widget.vtNumber,
                            awbNumber: _dl.MAWBNumber,
                            prefix: _dl.AirlinePrefix,
                          ))
                  //(
              ...
              if (returnVal != null) if (returnVal == true)
                getWarehouseAcceptanceAWBList(2);
            },
            child: Card(
              child: ListTile(
                leading: Container(
                        ...
                        children: [
                          Row(
                            children: [
                              ...
                              SizedBox(width: 10),
                              Text(_dl.AirlinePrefix + "-" + _dl.MAWBNumber,
...
// This function is called whenever the text field changes
  void _runFilter(String enteredKeyword) {
    List<WarehouseAcceptanceAWB> results = [];
    if (enteredKeyword.isEmpty) {
      ...
      if (enteredKeyword.isEmpty) {
        ...
        if (isSearched) {
          ...
          results.retainWhere((WarehouseAcceptanceAWB element) =>
