...
import 'package:luxair/otherpages/dockoutdetails.dart';
...
class _DockOutState extends State<DockOut> {
    ...
    _controllerModeType.addListener(() {
      setState(() {
        ...
        if (_controllerModeType.value) {
          checked = true;
          getDockOutTokenList(1); //Import
          dockInOutVTListToBind = dockInOutVTListImport;
        } else {
          checked = false;
          getDockOutTokenList(2); //Export
          dockInOutVTListToBind = dockInOutVTListExport;
        }
      });
    });
  ...
  getDockOutTokenList(modeType) async {
    if (isLoading) return;
    txtVTNO.text = "";
        ...
        ? GestureDetector(
            onTap: () async {
              var returnVal = await Navigator.push(
                context,
                MaterialPageRoute(
                    builder: (context) => DockOutDetails(
                    ...
                    useMobileLayout
                        ...
                        : Row(
                            ...
                            children: [
                              ...
                              SizedBox(width: 10),
                              Text(_dl.VEHICLENO.toUpperCase().toUpperCase(),
                                  style: VTlistTextFontStyleBoldiPad),
                            ],
                          ),
                    if (!useMobileLayout) SizedBox(height: 10),
